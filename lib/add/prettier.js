...
async function writePrettierrc(prettierrc) {
	...
	const prettierrcPath2 = path.join(process.cwd(), ".prettierrc.js");
	...
	const isExistsPrettierrcFile2 = checkProjectRcFile(".prettierrc");
	...
	if (isExistsPrettierrcFile || isExistsPrettierrcFile2) {
		const { action } = await inquirer.prompt([
			{
				name: "action",
				type: "list",
				message: `该项目已经存在 .prettierrc 文件`,
				choices: [
		...
		if (!action) {
			...
			await fs.remove(
				isExistsPrettierrcFile ? prettierrcPath2 : prettierrcPath
			);
		}
	}
